...
CREATE TABLE sales_transactions (
    ...
    refund_paid_cents INTEGER NOT NULL DEFAULT 0 CHECK (refund_paid_cents >= 0),
...
-- Record a cash/credit refund handed out by an administrator after a complete
-- dispense failure. The machine cannot safely command a remote hopper from the
-- dashboard, so this is an audited record of a physical handover.
...
BEGIN
    ...
    END IF;
    IF EXISTS (
        SELECT 1 FROM sales_transaction_lines
         WHERE transaction_id = p_transaction_id AND qty_dispensed > 0
    ) THEN
    ...
    VALUES ('INFO', 'DASHBOARD', 'FAILED_DISPENSE_REFUND_RECORDED',
            ...
            jsonb_build_object('refund_cents', v_amount));
