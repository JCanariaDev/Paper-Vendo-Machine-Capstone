...
export default function Login() {
  ...
  return (
    <div className="relative flex min-h-screen w-screen items-center justify-center p-4 md:p-8 bg-slate-50 overflow-hidden font-sans">
      {/* Premium Dynamic Background Elements */}
      <div className="absolute top-[-20%] left-[-20%] w-[600px] h-[600px] rounded-full bg-gradient-to-br from-primary-500/10 to-teal-500/5 blur-[130px] pointer-events-none animate-pulse duration-5000"></div>
      ...
      {/* Container for dual boxes */}
      <div className="flex flex-col md:flex-row items-stretch justify-center gap-8 max-w-5xl w-full relative z-10">
        
        {/* Minimal Info Box on the Left (hidden on mobile, visible on desktop) */}
        <div className="hidden md:flex flex-col justify-between w-full md:w-[460px] rounded-[32px] p-10 border border-slate-200/80 bg-white/90 backdrop-blur-md shadow-[0_20px_50px_rgba(15,23,42,0.06)] animate-[fadeIn_0.5s_ease-out]">
          <div className="flex flex-col items-start text-left">
            {/* Company Brand Logo and Text */}
            <div className="flex items-center gap-3.5 mb-10">
              <div className="w-14 h-14 rounded-2xl bg-white shadow-md border border-slate-100 p-2.5 flex items-center justify-center">
                <img src="/logo.png" alt="P&B V Machine Logo" className="w-full h-full object-contain" />
              </div>
              <div>
                <h2 className="font-display font-extrabold text-xl text-slate-800 tracking-tight">P&B V Machine</h2>
                <span className="text-xs font-bold text-primary-600 tracking-wide uppercase">Smart Dispensing System</span>
              </div>
            </div>

            {/* System Information */}
            <div className="space-y-6 mt-2">
              <h3 className="font-display font-extrabold text-2xl text-slate-800 leading-tight">
                Cloud Vendo Control Panel
              </h3>
              <p className="text-sm text-slate-500 leading-relaxed">
                Welcome to the centralized management dashboard for the Paper and Ballpen Vending Machine. Monitor physical inventory levels, view transaction histories, analyze product sales statistics, and configure operational settings remotely.
              </p>

              <div className="space-y-4 border-t border-slate-100 pt-6">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></div>
                  <span className="text-xs font-semibold text-slate-600">Machine Status: Online & Operational</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-primary-500"></div>
                  <span className="text-xs font-semibold text-slate-600">Hardware Controller: v2.4 (ESP32)</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-amber-500"></div>
                  <span className="text-xs font-semibold text-slate-600">Secure Database Sync: Active</span>
                </div>
              </div>
            </div>
          </div>

          {/* Footer info in Info Box */}
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-8">
            © {new Date().getFullYear()} P&B Vending Machine Capstone
          </div>
        </div>

        {/* Light Glass Card - Login Box (Right Side) */}
        <div className="w-full max-w-[440px] rounded-[32px] p-8 md:p-10 border border-slate-200/80 bg-white/90 backdrop-blur-md shadow-[0_20px_50px_rgba(15,23,42,0.06)] animate-[fadeIn_0.5s_ease-out]">
          
          {/* Brand Header */}
          <div className="flex flex-col items-center text-center mb-8">
            <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-white shadow-lg border border-slate-100 mb-4 transition-all duration-300 hover:scale-105 p-2.5">
              <img src="/logo.png" alt="P&B V Machine Logo" className="w-full h-full object-contain" />
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 rounded-xl bg-gradient-to-r from-primary-500 to-emerald-600 hover:from-primary-600 hover:to-emerald-700 disabled:opacity-75 font-semibold text-sm text-white shadow-lg shadow-primary-500/20 flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.98] mt-8"
            >
              {loading ? (
