...
/*
  updated_arduino_mega.ino - Fixed Production Version
  Master Controller for Paper & Pen Vendo
  UPDATED: Fixed logical error where DONE transaction ID was hardcoded to "1"
*/
...
void handleRequest(String type, String id) {
  ...
  isProcessing = true;
  currentRequestId = id; // Store selected ID globally
...
void performDispense(String msg) {
  ...
  if (totalSheets > 1) { // PAPER
    ...
    Serial1.println("DONE:paper:" + currentRequestId + ":" + name + ":" + String(cost) + ":" + String(totalSheets));
  } else { // PEN
    ...
    if (detected) {
      ...
      Serial1.println("DONE:pen:" + currentRequestId + ":" + name + ":" + String(cost) + ":1");
    } else {
