...
const int penHalfTurnSteps = stepsPerRevolution / 2;
...
void setup() {
  ...
  for (int i = 0; i < BALLPEN_COUNT; i++) {
    // Match the verified ULN2003 test setup and guarantee the coils are off
    // before the rest of the machine initializes.
    for (int coil = 0; coil < 4; coil++) {
      pinMode(penStopPins[i][coil], OUTPUT);
    }
    penSteppers[i]->setSpeed(10);
    stopStepper(i);
  }
